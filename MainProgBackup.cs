...
#pragma warning restore 0169
        ...
        public MainProg()
        {
            ...
            else
            {
                ...
                myRtbTermMessageLF("This can be obtained from (https://www.peak-system.com/PCAN-Basic.239.0.html?&L=1 ");
                myRtbTermMessageLF("and download PCAN Basic API. Otherwise do not use CAN features in UDT");
                myRtbTermMessageLF("**********************************************************************************");
            }
        ...
        private void MsgBoxCommandEntry()
        {
            ...
            {
                m_sEntryTxt = m_sEntryTxt.Replace("!", "");     // remove ! so that it goes to UART based channel. 
		...
		*/
        ...
        private void btnUDP_Connect_Click(object sender, EventArgs e)
        {
            ...
            if (myGlobalBase.ETH_isUDTClientOpen == false)
            {
                ...
                IPAddress IpAddress = IPAddress.Parse(tbUDPDesIP.Text);
                ...
                //-------------------------------------------Update Window Panel.
                myGlobalBase.ETH_udpClient = new UdpClient(iPortNoSrc);

                myRtbTermMessageLF("-I:Open UDP Client IPV4/Port:" + tbUDPDesIP.Text + ":" + tbUDPDesPort.Text);
                //-------------------------------------------Setup UDT Client.
                try
                {
                    //------------------------------------------- Setup UDPClient
                    myGlobalBase.ETH_udpClient.Connect(IpAddress, iPortNoSDsn);
                    //------------------------------------------- Setup Non Blocking Listening Port
                    IPEndPoint eee = new IPEndPoint(IPAddress.Any, 0);
                    UdpClient uuu = myGlobalBase.ETH_udpClient;  //new UdpClient(eee);

                    ETH_UdpState sss = new ETH_UdpState();
                    sss.e = eee;
                    sss.u = uuu;
                    uuu.BeginReceive(new AsyncCallback(ETH_ReceiveCallback), sss);
                    //-------------------------------------------Update state
        ...
        public void ETH_ReceiveCallback(IAsyncResult ar)
        {
            ...
            if (uuu.Client == null)     //###TASK, need to close async thread.
                return;

            byte[] receiveBytes = uuu.EndReceive(ar, ref eee);
            ...
            //myRtbTermMessageLF($"Received: {receiveString}");
            // }
            //----------------------------------------restart?
            ETH_UdpState sss = new ETH_UdpState();
            sss.e = eee;
        ...
        private void ETH_Close_UDPClientPort()
        {
            ...
            try
            {
                myGlobalBase.ETH_udpClient.Close();
                Tools.InteractivePause(TimeSpan.FromMilliseconds(500));
            }
            catch (Exception eeee)
            {
        ...
        private void CbETH_Ping_Click(object sender, EventArgs e)
        {
            ...
            if (myGlobalBase.ETH_isUDTClientOpen == false)
            {
                ...
                if (Tools.Ethernet_ValidateIPv4(tbUDPDesIP.Text) == false)
                {
                    mbOperationError = null;
                    mbOperationError = new DialogSupport();
                    mbOperationError.PopUpMessageBox("ERROR: Check IP address, it must be 4 x decimal (0 to 255) with comma", "IP Address", 30, 12F);
                    return;
                }
                ...
                try
                {
                    ...
                    PingReply reply = myPing.Send(tbUDPDesIP.Text, iPortNoSDsn);
                    if (reply != null)
                    {
                        myRtbTermMessageLF("-I: Ping Status :  " + reply.Status + " Time : " + reply.RoundtripTime.ToString() + " Address : " + reply.Address + ":" + iPortNoSDsn.ToString());
                    }
                }
                catch
                {
